...
<script>
        $(document).ready(function () {
			@if(Session::has('update'))
				...
				toastr.success('Instansi berhasil diubah.');
			...
			@if(Session::has('delete'))
				...
				toastr.success('Instansi berhasil dihapus.');
			...
			var div = $(event.relatedTarget) // Tombol dimana modal di tampilkan
				...
				modal.find('#hapus-true').attr("href","{{ action('InstansiController@getDelete') }}/"+id);
			 
			})
			
			$.getJSON( '{{ action("InstansiController@getData") }}', function( mydata ) {
            }).done(function(mydata){
			
				
				function linkEdit(cellValue, options, rowdata, action)
				{
					return "<a href='{{ action('InstansiController@getEdit') }}/" + rowdata.id + "'><span class='ui-icon ui-icon-pencil'>&nbsp;&nbsp;&nbsp;</span></a>";
				}
				...
				$("#table_list_1").jqGrid({
					...
					rowNum: 14,
					rowList: [10, 20, 30],
					colNames: ['Nama Instansi', 'Alamat', 'Edit', 'Delete'],
					colModel: [
						{name: 'nama', index: 'nama', width: 90, align: 'center'},
						{name: 'alamat', index: 'alamat', width: 80},
		...
		<div class="col-lg-12">
			<div class="ibox">
				...
				<div class="ibox-content">
					<a href="{{ action('InstansiController@getEntry') }}" class="btn btn-primary btn-sm">+ Tambah Instansi</a>
	...
	<div class="modal-dialog">
		<div class="modal-content animated fadeIn">
			...
			<div class="modal-header">
				...
				<small>Menghapus data instansi</small>
			</div>
